...
namespace SimpleAUTH.Controllers
{
    ...
    public class FlashcardSetController : BaseController
    {
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult<InsertFlashcardsIntoSetDTO>> UpdateFlashcardSet(int id, InsertFlashcardsIntoSetDTO updatedFlashcardSet)
        {
            int userId = CurrentUserId;

            // creates a new set and maps the flashcard from the DTO to a new set
            var set = new FlashcardSet
            {
                Id = id,        // flashcard set ID
                Flashcards = updatedFlashcardSet.Flashcards.Select(f => new Flashcard
                {
                    UserId = userId,            // maps the flashcard directly to user's ID.
                    Id = 0,                     // Id is set to 0, because it will be generated by the database when the flashcard is create
                    FlashcardSetId = id,        // using the id of the flashcard set here, directly form the URL
                    Front = f.Front,
                    Back = f.Back
                }).ToList()
            };

            var result = await _flashcardSetService.UpdateFlashcardSet(userId, id, set);
            if (result == null)
                return NotFound();
